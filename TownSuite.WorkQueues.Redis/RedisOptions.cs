...
public class RedisOptions : BatchOptions
{
    ...
    /// Defaults to <c>{MachineName}-{ProcessId}</c> so that multiple processes on the
    /// same host get distinct pending-entry lists and do not reclaim each other's messages.
    /// Override this when you need a stable, predictable name (e.g. in tests).
    /// </summary>
